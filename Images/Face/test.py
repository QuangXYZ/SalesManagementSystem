...
def random_emp_id(stringLength):
    Digits = string.digits#lấy chuỗi số từ 3 - 9
...
class Employee:
    def __init__(self, top=None):
        ...
        self.entry1.configure(relief="flat")#làm mất viền của entry

        self.button1 = Button(root)
        self.button1.place(relx=0.229, rely=0.289, width=76, height=23)
        self.button1.configure(relief="flat")
        self.button1.configure(overrelief="flat")#không hiển thị viền khi chuột di chuyển qua
        self.button1.configure(activebackground="#CF1E14")
        self.button1.configure(cursor="hand2")#kiểu con trỏ chuột di qua
        self.button1.configure(foreground="#ffffff")#màu chữ
        ...
        self.tree.configure(selectmode="extended")# cho phép chọn nhiều item, shift, ctrl
    ...
    def add_emp(self):
        global e_add
        e_add = Toplevel()# Tạo ra cửa sổ mới
        page6 = add_employee(e_add)
        e_add.protocol("WM_DELETE_WINDOW", self.ex)#được sử dụng để xử lý sự kiện khi người dùng đóng cửa sổ
        e_add.mainloop()

    def ex(self):
        e_add.destroy()
        self.tree.delete(*self.tree.get_children())# xóa dữ liệu trên table
        self.DisplayData()
    ...
    sel = []
    def on_tree_select(self, Event):# hàm dùng để thêm các mục đã chọn trên bảng và lưu vào sel
        self.sel.clear()
    ...
    def update_emp(self):
        ...
        if len(self.sel) == 1:
            ...
            global vall
            vall = []
            for i in self.sel:# lấy giá trị từ các ô trong một bảng để hiển thị lên giao diện update
